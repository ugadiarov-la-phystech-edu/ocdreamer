...
export COMET_PROJECT_NAME=homegrid_flatten_slots_ocdreamer
export COMET_EXPERIMENT_NAME=model-${model}_task-${task}_seed-${seed}_flatten_slots_onehot
#export COMET_RUN_ID=$COMET_RUN_ID_HOMEGRID

SLOT_EXTRACTOR_TYPE="slotcontrast"           
SLOT_EXTRACTOR_CONFIG="$SLOT_EXTRACTOR_CONFIG_HOMEGRID"
SLOT_EXTRACTOR_CHECKPOINT="$SLOT_EXTRACTOR_CHECKPOINT_HOMEGRID"
...
export CUDA_VISIBLE_DEVICES=$device; python dreamerv3/main.py  --configs dynalang homegrid \
    ...
    \
    --agent.enc.resnet.vec_keys 'token$|flatten_slots$' \
    --agent.enc.resnet.img_keys '$^' \
    --agent.enc.resnet.pass_keys 'flatten_slots' \
    --agent.dec.resnet.vec_keys 'token$|flatten_slots$' \
    --agent.dec.resnet.img_keys '$^' \
    --agent.dec.resnet.vec_dists 'token:onehot,flatten_slots:mse' \
