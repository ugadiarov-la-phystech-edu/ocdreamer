...
register('lift-v0', entry_point='embodied.envs.robosuite_env:RobosuiteEnv', kwargs=dict(task='lift'))
register('lift-medium-v0', entry_point='embodied.envs.robosuite_env:RobosuiteEnv', kwargs=dict(task='lift-medium'))
register('lift-medium-raw-v0', entry_point='embodied.envs.robosuite_env:RobosuiteEnv', kwargs=dict(task='lift-medium-raw'))
...
class RobosuiteEnv(gym.Env):
	...
	def __init__(self, task, timelimit, size, seed):
		...
		self._horizon = timelimit['duration'] + 1
	...
	def _process_observation(self, observation):
		...
		self._last_frame = np.array(Image.fromarray(image).resize(shape[:2]))
	...
	def step(self, action):
		...
		return self._process_observation(observation), reward, False, False, info
